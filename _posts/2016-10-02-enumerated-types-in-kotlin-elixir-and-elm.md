...
The goal of this blog post is to define the concept of an enumerated type and
...
introduce and define enumerated types. Then, in
...
a pseudo [ML](https://en.wikipedia.org/wiki/ML_(programming_language))-like
language as:
...
where we declare a `type` with the name `Boolean` and
...
case bool of
    True ->
        -- do this if bool == True

    False ->
        -- do this if bool == False
{% endhighlight %}

where we case on the variable `bool` and add a clause for each of the
members/type constructors, `True` and `False`.

For practical reasons, a boolean type is usually included by default in most
modern programming languages, so in the following sections we look at how to
express another enumerated type, `Color`, with members `Red`, `Green`, and
`Blue` in our three programming languages of choice.
...
where we define a function, `toHex`, which takes an argument of type `Color` and
...
expression for pattern matching and as in our `Boolean` example, the body of the
expression includes a clause for each of the members of the enum type (class).

Before we move on, there are a few things worth noting about the above code
snippets:

- Kotlin allows us to separate data, `Color`, and logic, `toHex`, such that we
  do not have to introduce the concept of converting to hex color codes into our
  enum class as a method, but instead we can define a separate function,
  `toHex`, somewhere else in the code, resulting in
  lower
...
-  since `toHex` takes an argument of type `Color` rather than `Color?`, the
   ...
   `null` reference, which helps make Kotlin code much easier to reason about
   than Java code.

Finally, in order to test the above code, we write a `main` function which
instantiates a `Color` variable, `blue`, and prints the result of calling
`toHex` on it:
...
Having now shown how we implement an enum type, `Color`, in Kotlin and pattern
match on its members, we move on to repeat the exercise in Elixir.
...
`Color`, returns a string representation of the hex color code corresponding to
the original `Color` value, via pattern matching:
...
version, there is one distinctive difference: we cannot 100% guarantee on
runtime that `to_hex` is never given an argument that is not of type `Color`, in
which case if we don't include an `else` clause in the `case` expression, Elixir
will throw a runtime error. However, by specifying proper type signatures of our
functions combined with Elixir's excellent type inference engine and tools like
dialyxir, all of which we have discussed above, we can do much to reduce this
risk.

Finally, do note that we could also have written `to_hex` in a slightly more
idiomatic way:
...
Having implemented our `Color` enum in both Kotlin and Elixir, we move on to
our final language example, Elm.

### 5. Enumerated types in Elm

In this section, we implement the enumerated type `Color` in Elm.

In the case of Elm, we return to the ML-inherited syntax we saw at the beginning
of this post, where we define our type, `Color`, using the `type` keyword
...
`else`-like clause in our `case` expression, as the `Color` type can only be
...
Finally, in order to run the above code, we implement the `main` function, which
creates a `text` DOM element containing a string describing the hex color code
of the value `Blue`.
...
Having implemented our `Color` enum in our third and final language, Elm, we
conclude this post in the next section.

### 6. Conclusion

In this blog post, we have defined the concept of an enumerated type, and
compared the implementation of an enumerated type in the three different
programming languages: Kotlin, Elixir, and Elm.

Across the three implementations we notice that Elixir is the only language
which does not have a dedicated keyword/construct for defining enumerated types
